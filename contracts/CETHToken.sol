...
contract CETHToken is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
    /**
     ...
     */
    ETHInterestRateModel public interestRateModel;
     ...
     */
    event NewMarketInterestRateModel(ETHInterestRateModel oldInterestRateModel, ETHInterestRateModel newInterestRateModel);
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setInterestRateModel(ETHInterestRateModel newInterestRateModel) public returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setInterestRateModelFresh(ETHInterestRateModel newInterestRateModel) internal returns (uint) {

        // Used to store old model for use in the event that is emitted on success
        ETHInterestRateModel oldInterestRateModel;
